...
public class DynamicPagerIndicator extends LinearLayout implements ViewPager.OnPageChangeListener {
    ...
    public void initDynamicPagerIndicator(Context context, AttributeSet attributeSet) {
        ...
        if (typedArray != null) {
            ...
            mTabNormalTextSize = typedArray.getDimension(R.styleable.DynamicPagerIndicator_tabNormalTextSize, IndicatorUtil.sp2px(mContext, 18));
            mTabSelectedTextSize = typedArray.getDimension(R.styleable.DynamicPagerIndicator_tabSelectedTextSize, IndicatorUtil.sp2px(mContext, 18));
    ...
    public void tabTitleColorGradient(int position, float positionOffset) {
        if (mTabParentView != null && position < mTabParentView.getChildCount()) {
            ...
            if (pageTabView != null) {
                pageTabView.getTitleTextView().setTextColor(IndicatorUtil.evaluateColor(mTabSelectedTextColor, mTabNormalTextColor, positionOffset));
            }
            PagerTabView afterPageTabView = (PagerTabView) mTabParentView.getChildAt(position + 1);
            if (afterPageTabView != null) {
                afterPageTabView.getTitleTextView().setTextColor(IndicatorUtil.evaluateColor(mTabNormalTextColor, mTabSelectedTextColor, positionOffset));
            }
        }
    }
    ...
    public void transactionScrollTitleParentToCenter(int position) {
        ...
        final int halfScreenWidth = IndicatorUtil.calculateScreenWidth(mContext) / 2;
    ...
    public void calculateIndicatorLineWidth() {
        if (mPagerIndicatorMode == INDICATOR_MODE_SCROLLABLE || mPagerIndicatorMode == INDICATOR_MODE_SCROLLABLE_CENTER) {
            ...
            if (mIndicatorLineWidth == 0) {
                mIndicatorLineWidth = IndicatorUtil.calculateScreenWidth(mContext) / mTabParentView.getChildCount();
            }
        }
    }
    ...
    public LinearLayout createTabParentView() {
        ...
        LayoutParams layoutParams = new LayoutParams(
                mPagerIndicatorMode == INDICATOR_MODE_SCROLLABLE_CENTER ? IndicatorUtil.calculateScreenWidth(mContext) : LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
    ...
    pagerIndicatorMode : 指示器的显示模式，共有三种。 1、scrollable：适用于ViewPager的count较多时。可滑动。默认从左向右排列显示 2、scrollable_center：居中显示，适用于ViewPager的count较少时，且需要居中显示 3、fixed：均分模式，该模式下会平均分配TabView的宽度
    ...
    indicatorLineScrollMode：其为TabView下的导航条的滚动模式，共有两种 1、dynamic：即爱奇艺/腾讯视频那种可变化长度的效果。导航条长度、位置均变化。 2、transform：普通移动效果，导航条长度不变，位置变化。


     <!--
     <com.htdata.goyo.view.indicator.DynamicPagerIndicator
        android:id="@+id/md_indicator"
        app:indicatorLineEndColor="@color/blue_bg3"
        app:indicatorLineHeight="2dp"
        app:indicatorLineRadius="3.0dp"
        app:indicatorLineScrollMode="transform"
        app:indicatorLineStartColor="@color/blue_bg3"
        app:indicatorLineWidth="@dimen/qb_px_20"
        app:pagerIndicatorMode="scrollable"
        app:tabNormalTextColor="@color/gray3"
        app:tabNormalTextSize="12sp"
        app:tabSelectedTextColor="@color/blue_bg3"
        app:tabSelectedTextSize="12sp"
