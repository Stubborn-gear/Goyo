...
public class ConversionUtil {
    ...
    public static String getPercent(double num){
        ...
        LogUtils.v("=======百分比数据转换======="+format);
        double number = toDouble(format)*100;
        LogUtils.v((int)number+"=======百分比数据转换======="+number);
        return (int)number+"";
    }

    /** 我的设备  百分比数据转换 不四舍五入 */
    public static String getPercent2(double num){
        DecimalFormat formater = new DecimalFormat();
        formater.setMaximumFractionDigits(2);
        formater.setGroupingSize(0);
        formater.setRoundingMode(RoundingMode.FLOOR);
        String result = formater.format(num);
        double number = toDouble(result)*100;
        return (int)number+"";
    }
     ...
     * @return
     */
    public String doubleToString2(double num){
