...
//                attributes.flags |= flagTranslucentStatus;
    ...
    public static void setStatusColor(Activity mActivity) {
        StatusBarUtils.fitsSystemWindows(mActivity);
    ...
    public static void setStatusBarLightMode(Window window) {
        ...
        if (type == 1) {
            LogUtils.v("===设置小米===黑色字体=======");
            MIUISetStatusBarLightMode(window, true);
        } else if (type == 2) {
            LogUtils.v("===设置魅族===黑色字体=======");
    ...
    public static void StatusBarDarkMode(Window window) {
        int type = getStatusBarLightMode(window);
        if (type == 1) {
            LogUtils.v("===清除小米===黑色字体=======");
            MIUISetStatusBarLightMode(window, false);
        } else if (type == 2) {
            LogUtils.v("===清除魅族===黑色字体=======");
    ...
    private void setStatusBarFontIconDark(boolean dark) {
        ...
        try {
            ...
            Method extraFlagField = clazz.getMethod("setExtraFlags", int.class, int.class);
            if (dark) {    //状态栏亮色且黑色字体
                LogUtils.v("===小米====状态栏亮色且黑色字体==============");
                extraFlagField.invoke(window, darkModeFlag, darkModeFlag);
            } else {       //清除黑色字体
                LogUtils.v("=====小米==清除黑色字体==============");
        ...
        try {
            ...
            if (dark) {
                value |= bit;
                LogUtils.v("====魅族===状态栏亮色且黑色字体==============");
            } else {
                value &= ~bit;
                LogUtils.v("=====魅族==清除黑色字体==============");
            }
            meizuFlags.setInt(lp, value);
            window.setAttributes(lp);
        } catch (Exception e) {
...
//===============================状态栏字体颜色设置=======结束============================================
     ...
     */
    public static void initImmersionBar(Activity mActivity, boolean isDarkFont) {
        if (isDarkFont) {
